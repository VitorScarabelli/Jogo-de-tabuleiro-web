...
<head>
  ...
  <style>
    ...
    .nuvens .titulo {
      ...
      background: none; /* tira o fundo vermelho */
    }
...
<body>
  ...
  <div class="nuvens">
    ...
    <h2 class="titulo">Escolha seus personagens</h2>
  </div>
    ...
    <div class="nuvens">
      <img src="./imageTabuleiro/nuvens.png" alt="Nuvem">
      <h2 class="titulo">Escolha eventos e casas:</h2>
    </div>

    <!-- Accordion principal -->
    <div class="accordion">
      <?php foreach ($eventosPorDificuldade as $dif => $eventos): ?>
          ...
          <div class="accordion-content">
            <div class="evento-grid">
              <?php foreach ($eventos as $row): ?>
                <?php
                // Define classe de cor (bom ou ruim)
                $classeCard = ($row['impactoEvento'] === 'ruim') ? 'card card-ruim' : 'card card-bom';
                ?>
                <div class="<?= $classeCard ?>">
                  <div class="card-body">
                    <div>
                      <input type="checkbox" name="eventos[<?= $row['idEvento'] ?>][ativo]" value="1" id="evento<?= $row['idEvento'] ?>">
                      <label for="evento<?= $row['idEvento'] ?>">
                        <strong><?= htmlspecialchars($row['nomeEvento']) ?></strong>
                      </label>
                    </div>

                    <p><strong>Descrição:</strong> <?= htmlspecialchars($row['descricaoEvento']) ?></p>
                    <p><strong>Modificador:</strong> <?= htmlspecialchars($row['modificadorEvento']) ?></p>

                    <label for="casa<?= $row['idEvento'] ?>">Casa no tabuleiro:</label>
                    <input type="number" id="casa<?= $row['idEvento'] ?>"
                      name="eventos[<?= $row['idEvento'] ?>][casa]" min="1" max="50">
                    ...
                    <input type="hidden" name="eventos[<?= $row['idEvento'] ?>][casas]" value="<?= intval($row['casaEvento']) ?>">
                  </div>
                </div>
              <?php endforeach; ?>
            </div>
    ...
    <!-- Botões gerais -->
    <button type="submit" name="modoNormal" value="1">Confirmar Seleção Manual</button>
    <button type="submit" name="modoAleatorio" value="1">Atribuir Casas Aleatórias</button>
    <!-- <button type="submit" name="modoAutomatico" value="1">Modo Automático (Todos Eventos)</button>
    !-->
  </form>
  <script>
    const maxJogadores = 4; // limite fixo
      ...
      p.addEventListener("click", () => {
        if (p.classList.contains("selecionado")) {
          ...
          if (selecionados.length < maxJogadores) {
            ...
            alert("Você já selecionou o número máximo de jogadores (4).");
            return;
          }
        }
    ...
    form.addEventListener("submit", e => {
      if (selecionados.length < 1) {
        e.preventDefault();
        alert("Selecione pelo menos 1 personagem antes de continuar!");
      } else {
        ...
        document.getElementById("hidden-personagens").value = JSON.stringify(personagensData);
      }
    });
  </script>
  <script>
